...
void main() {
	...
	float h = 1.0/ height;
 	...
 	vec3 center_left   = texture(image, texcoord + vec2(-w, 0.0)).rgb;
 	vec3 center_center = texture(image, texcoord).rgb;
 	vec3 center_right  = texture(image, texcoord + vec2(w, 0.0)).rgb;
